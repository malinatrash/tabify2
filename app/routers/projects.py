...
from ..models import User, Project, ProjectShare, Notification, ProjectLike, MidiFile, Tablature
...
from ..utils.midi_to_tab import midi_to_tablature, save_tablature, convert_midi_to_tablature
...
async def upload_audio(
    ...
    project = get_project_with_access_check(project_id, current_user, db, allow_public=False)
    if not project:
        return JSONResponse(
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                with open(temp_file_path, "rb") as file_data:
                    ...
                    print(f"Отправка файла {file.filename} на обработку в basic-pitch...")
                    response = await client.post("http://host.docker.internal:8001/audio-to-midi", files=files)
                
                print(f"Получен ответ от basic-pitch: статус {response.status_code}")
                if response.status_code != 200:
                    # Попробуем получить детали ошибки
                    try:
                        error_content = response.json() if response.headers.get("content-type") == "application/json" else {"error": "Ошибка при обработке файла"}
                    except Exception as json_err:
                        error_content = {"error": f"Ошибка при обработке файла: {str(response.content)[:100]}..."}
                    
                    print(f"Ошибка при обработке в basic-pitch: {error_content}")
                    return JSONResponse(
...
async def delete_midi_file(
    ...
    if not project:
        return JSONResponse(
            content={"error": "Проект не найден или у вас нет прав для его редактирования"},
            status_code=403
        )
...
async def generate_tab_page(project_id: int, midi_id: int, request: Request, current_user: Optional[User] = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    project = get_project_with_access_check(project_id, current_user, db)
    if not project:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Проект не найден или у вас нет к нему доступа")
    
    # Проверка наличия MIDI-файла
    midi_file = db.query(MidiFile).filter(MidiFile.id == midi_id, MidiFile.project_id == project_id).first()
    if not midi_file:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="MIDI-файл не найден")
    
    # Проверяем, есть ли уже табулатура для этого MIDI
    existing_tab = db.query(Tablature).filter(Tablature.midi_file_id == midi_id).first()
...
async def view_tab(project_id: int, midi_id: int, request: Request, current_user: Optional[User] = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    tablature = db.query(Tablature).filter(Tablature.midi_file_id == midi_id).first()
    if not tablature:
...
async def update_tab(project_id: int, midi_id: int, request: Request, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not project:
        return JSONResponse(
            ...
            content={"error": "Только владелец проекта может редактировать табулатуру"}
        )
